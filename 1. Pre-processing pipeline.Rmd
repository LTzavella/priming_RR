...
* Participants with error rates (ERs) greater or equal to 0.4 in either food or non-food prime trials 
  will be excluded from all respective analyses
  
```{r}
#Get IDs of participants that will be included in analyses based on ER < 0.4
ER1_incl <- unique(c(unique(nonfood1_app$.id[nonfood1_app$ER<0.4]), unique(food1_app$.id[food1_app$ER<0.4])))
ER2_incl <- unique(c(unique(nonfood2_app$.id[nonfood2_app$ER<0.4]), unique(food2_app$.id[food2_app$ER<0.4])))

#Remove participants from analyses datasets and data1/data2 which will be later pre-processed for FCT analyses
nonfood1_app <- subset(nonfood1_app, !(ER>=0.4))
nonfood2_app <- subset(nonfood2_app, !(ER>=0.4))

food1_app <- subset(food1_app, !(ER>=0.4))
food2_app <- subset(food2_app, !(ER>=0.4))

data1 <- data1[sapply(data1, function(x) any(x$subject %in% ER1_incl))]
data2 <- data2[sapply(data2, function(x) any(x$subject %in% ER2_incl))]
```
...
#Online data 
food2_app$diff_RT = food1_app$con_RT - food1_app$inc_RT
food2_app$diff_RT_h = food1_app$con_h_RT - food1_app$inc_h_RT
food2_app$diff_RT_u = food1_app$con_u_RT - food1_app$inc_u_RT
food2_app$diff_DRT = food1_app$DRT_h - food1_app$DRT_u
